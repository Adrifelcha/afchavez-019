...
for (i in 1:k){
      ...
      delta ~ dnorm(0,0.7)
      MuC ~ dnorm(0,0.7)
      delta_prior ~ dnorm(0,0.7)
      MuC_prior ~ dnorm(0,0.7)}
      ','0-diff_dprime.bug')
...
myinits <- list(
  list(d_A = rep(0,k), d_B = rep(0,k), c_A = rep(1,k),c_B = rep(1,k), lambdac_A = 1,  lambdac_B = 1, 
       delta = 0, MuC = 0))

# Identificamos los parámetros a inferir
parameters <- c("c_A", "c_B", "d_A", "d_B", "thetah_A", "thetah_B", "thetaf_A", "muc_A", "muc_B", 
                "thetaf_B", "sigmac_A", "sigmac_B", "delta", "MuC", "delta_prior", "MuC_prior")
...
numero <- 0
linea <- 1.75
plot(x_axis_a, y_axis_a, ann=F, axes=F,cex=0.5, pch=1, col="deepskyblue3", ylim=c(-1.5,1.5))
...
for(u in 1:20){
  ...
  lines(c(linea,linea),c(-1.5,1.5), lty=2)
  linea <- linea+1
  }
...
axis(2,seq(-1.5,1.5,0.5),seq(-1.5,1.5,0.5), line=-1)
legend(4,5.5, legend=c("A stimuli", "B stimuli"),
       col=c("deepskyblue3", "darkorchid3"), lty=1, cex=0.8)
title("Posterior densities of C estimates per participant", cex=2)