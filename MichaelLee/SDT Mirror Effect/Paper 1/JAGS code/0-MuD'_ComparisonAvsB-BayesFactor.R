...
parameters <- c("c_A", "c_B", "d_A", "d_B", "thetah_A", "thetah_B", "thetaf_A", "mud_A", "mud_B", 
                "thetaf_B", "sigmad_A", "sigmad_B", "delta", "MuD", "delta_prior", "Pr_delta_D")
...
Prior_Delta <- samples$BUGSoutput$sims.list$Pr_delta_D
...
layout(matrix(1:1,ncol=1))
  ...
  layout(matrix(1:1,ncol=1))
  plot(density(Delta), col='blue4', lwd=2, ann=F, axes=F, xlim=c(-0.5,2))
  #lines(seq(-100,100,.05), dnorm(seq(-100,100,.05), 0,1), lwd=1, col="darkorchid3")
  lines(density(Prior_Delta), lwd=2, col="red")
  axis(1)
  axis(2, line=.5)
  ...
  mtext(side=3, paste("Experiment No.", exp), cex=3,line=0.5, f=2)
  ...
  text(0,0.95,paste(round(SavageDickey,3)), f=2, cex=2.7)  
  text(0,1.1,paste("Bayes Factor"),f=1, cex=3)

  
  #### Estimating Actual densities
  max_prior <- max(density(Prior_Delta)$y[which(density(Prior_Delta)$x<0.01&density(Prior_Delta)$x>-0.01)])
  max_post <- max(density(Delta)$y[which(density(Delta)$x<0.01&density(Delta)$x>-0.01)])
  SavDick_2 <- max_prior/max_post 
