...
model{
  ...
  # Posterior density based on our data
    ...
    gamFA ~ dgamma(.001,.001)T(0.05,.1)
    gamFB ~ dgamma(.001,.001)T(0.05,.1)
    gamHA ~ dgamma(.001,.001)T(0.05,.1)
    gamHB ~ dgamma(.001,.001)T(0.05,.1)
    sdFA <- 1/sqrt(gamFA)
    sdFB <- 1/sqrt(gamFB)    
    sdHA <- 1/sqrt(gamHA)    
    sdHB <- 1/sqrt(gamHB)    
    ...
    muHB <- muH - deltaH/2
    muFB <- muF + deltaF/2
    deltaH ~ dbeta(1,1)T(0.5,.95)
    deltaF ~ dbeta(1,1)T(0.05,0.5)
    muH ~ dbeta(1,1)T(0.5,0.95)
    muF ~ dbeta(1,1)T(0.05,0.5)
#PRIORS
for (i in 1:k){
  # Posterior density based on our data
    ...
    Pr_deltaH ~ dbeta(1,1)T(0.5,0.95)
    Pr_deltaF ~ dbeta(1,1)T(0.05,0.5)
    Pr_muH ~ dbeta(1,1)T(0.5,0.95)
    Pr_muF ~ dbeta(1,1)T(0.05,0.5)
      }','Delta-Rate.bug')
