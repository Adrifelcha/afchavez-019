...
# Or use a heatmap:
library(ks)
d <- kde(cbind(n, theta), xmin = rep(0, 2), xmax=c(500, 1)) # fit multivariate density estimator
image(d$eval.points[[1]], d$eval.points[[2]], d$estimate, xlab="Number of Surveys", ylab="Rate of Return", las=1) # heatmap
# An alternative way is a contour plot (code not shown)
