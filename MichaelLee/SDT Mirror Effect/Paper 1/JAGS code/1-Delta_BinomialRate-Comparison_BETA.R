...
Exp <- 1}else{
  ...
  ###############################
    # THETA HITS:   
          #Prior Distribution
  plot(soporte_h, col="white", main="", cex.main=3, ylab="", xlab="", xlim=c(0.45,1), axes=F,
       ylim=c(0,3))
  for(a in 1:k){
    axis(1)
    ...
    legend(0.6,2.8, legend=c("A Class", "B Class"),
  ...
  mtext(paste("Prior distribution for the Hit rates - Experiment No.", Exp), font=2, cex=2, side=3)
          #Predictive Prior
  plot(soporte_h, col="white", main="", cex.main=3, ylab="", xlab="", xlim=c(0.45,1), 
       axes=F, ylim=c(0,27))
  for(a in 1:k){
    axis(1)
    axis(2, labels=F, at=c(0,94))
    ...
    legend(0.6,20, legend=c("A Class", "B Class"),
           col=c("dodgerblue2", "darkorchid2","dodgerblue4", "darkorchid4"), lty=1, cex=1.2, lwd=c(2,2,5,5))
  ...
  for(a in 1:k){
    ...
    axis(1)
    axis(2, labels=F, at=c(0,94))
    legend(0.35,2.7, legend=c("A Class", "B Class"),
           col=c("dodgerblue2", "darkorchid2","dodgerblue4", "darkorchid4"), lty=1, cex=1.2, lwd=c(2,2,5,5))
