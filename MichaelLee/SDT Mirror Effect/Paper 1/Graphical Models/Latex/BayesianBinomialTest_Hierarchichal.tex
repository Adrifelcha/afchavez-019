...
\rput(3.5,11){\GM@node[nodeSize=15mm,
					%observed=true,
...
\rput(3,8.5){\GM@node[nodeSize=17mm,
					%observed=true,
					observedColor=lightgray,
					%query=true
					]{MuA}}
				\rput(3,8.5){\GM@node[nodeSize=14mm]{Q_small}}
				\GM@label[offset=-3.5mm]{MuA}{\huge$\mu^A_{i}$}

\rput(6.5,8.5){\GM@node[nodeSize=15mm,
					%observed=true,
					observedColor=lightgray,
					%query=true
					]{SigmaA}}
				\GM@label[offset=-2.5mm]{SigmaA}{\huge$\sigma^A_{i}$}
\rput(7.5,8.5){\GM@node[nodeSize=17mm,
					%observed=true,
					observedColor=lightgray,
					%query=true
					]{MuB}}
				\rput(7.5,8.5){\GM@node[nodeSize=14mm]{Q_small}}
				\GM@label[offset=-3.5mm]{MuB}{\huge$\mu^B_{i}$}

\rput(9,8.5){\GM@node[nodeSize=15mm,
					%observed=true,
					observedColor=lightgray,
					%query=true
					]{SigmaB}}
				\GM@label[offset=-2.5mm]{SigmaB}{\huge$\sigma^B_{i}$}
...
\rput(1.40, 0.7){\GM@plate[plateLabelPos=br]{8.2}{7.3}{\Large$i$ Stimuli (Signal or Noise)}}
...
\rput(11.5,11){\pnode{equation}}\GM@label{equation}{\huge$\theta_{h} \sim \mathrm{Uniform}(0.5,1)$}

\rput(11.5,10){\pnode{equation}}\GM@label{equation}{\huge$\theta_{fa} \sim \mathrm{Uniform}(0,0.5)$}

\rput(11.5,9){\pnode{equation}}\GM@label{equation}{\huge$\delta \sim \mathrm{Uniform}(0,0.5)$}


\rput(11.5,7.5){\pnode{equation}}\GM@label{equation}{\huge$\theta^A_{h} \gets \theta_{i} + \frac{\delta}{2}$}

\rput(11.5,6.5){\pnode{equation}}\GM@label{equation}{\huge$\theta^B_{h} \gets \theta_{i} - \frac{\delta}{2}$}

\rput(11.5,5.5){\pnode{equation}}\GM@label{equation}{\huge$\theta^A_{fa} \gets \theta_{i} - \frac{\delta}{2}$}

\rput(11.5,4.5){\pnode{equation}}\GM@label{equation}{\huge$\theta^B_{fa} \gets \theta_{i} + \frac{\delta}{2}$}



\rput(11.5,0){\pnode{equation}}\GM@label{equation}{\huge$K^A_{i}\sim \mathrm{Binomial}(\theta^A_{i}, n^A)$}
